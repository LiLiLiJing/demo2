...
$(function () {
    ...
    $(".first_dt img").attr("src", "static/images/left/select_xl.png"); //当前焦点一级菜单项图标
    ...
    $(".leftsidebar_box dt").click(function () {
        ...
        if ($(this).parent().find('dd').is(":hidden")) {
            ...
            $(this).parent().find('img').attr("src", "static/images/left/select_xl.png"); //当前焦点一级菜单项图标                 
        }
        else {
            $(this).parent().find('dd').slideUp(); //滑动方式隐藏子菜单
            $(this).css({ "background-color": "#339999" }); //非焦点一级菜单项背景颜色
            $(this).parent().find('img').attr("src", "static/images/left/select_xl01.png"); //非焦点一级菜单项图标
        }
    });
    ...
    //                $(this).css({ "background-color": "#1f6b75" }); //焦点一级菜单项背景颜色             
    //                $(".leftsidebar_box dt img").attr("src", "static/images/left/select_xl01.png"); //非焦点一级菜单项图标
    //                $(this).parent().find('img').attr("src", "static/images/left/select_xl.png"); //当前焦点一级菜单项图标
...
/**系统初始默认页面源码显示 **/
$(function () {
    setCore("uploadpage"); //显示默认页面的源码
})

/** 二级菜单项对应功能页面的源码显示 **/
function setCore(name) {
    var service_name = name;
    var htmlUrl = service_name; //请求的页面
    ...
    // alert(htmlString);
    // $('#container_iframe').load(htmlString); //设置右侧容器的页面地址   
    // $('#container_iframe').body.innerHTML = htmlString;
